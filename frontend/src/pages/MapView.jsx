        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-sm space-y-3">
            ...
            <p className="text-xs text-slate-500">
              Corridors classified by weighted incident severity and multi-factor hazard scores.
            </p>
            ...
            {hotspots.length === 0 ? (
              <div className="p-6 text-center text-slate-400 bg-slate-50 rounded-2xl">
                ...
                <p className="text-xs">No hotspot clusters formed yet.</p>
              </div>
            ) : (
              <div className="space-y-2.5 max-h-[520px] overflow-y-auto pr-1">
                {hotspots.map((hs, idx) => (
                  <div
                    ...
                    className="p-3.5 rounded-2xl bg-slate-50/70 border border-slate-200/80 hover:border-sky-300 transition cursor-pointer space-y-2 hover:bg-slate-50"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-xs text-slate-800">{hs.location}</span>
                      <SeverityBadge severity={hs.risk_level === 'SEVERE' ? 'CRITICAL' : hs.risk_level} />
                    </div>

                    <div className="flex items-center justify-between text-xs text-slate-500">
                      <span>Total Defects: <strong className="text-slate-800 font-mono">{hs.event_count}</strong></span>
                      <span>Danger Score: <strong className="text-amber-600 font-mono">{hs.severity_score}</strong></span>
                    </div>
                    ...
                    {hs.primary_event_types && hs.primary_event_types.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {hs.primary_event_types.map((type, tIdx) => (
                          <span
                            key={tIdx}
                            className="bg-white px-2 py-0.5 rounded-md text-[10px] text-slate-600 border border-slate-200 font-medium"
                          >
                            {type.replace(/_/g, ' ')}
                          </span>
                        ))}
